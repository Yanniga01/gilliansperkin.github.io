...
2. Add the new URL to `sitemap.xml` with realistic `<lastmod>`.
3. Add internal links from one related article and one related category page.
4. (Optional in separate PR) Add the new article card/link to `/blog` (`blog/index.html`) to reduce merge conflicts.
...
- `/blog`
- one category page (`/categories/finance`, `/categories/pdf-tools`, `/categories/project-management`, `/categories/email-marketing`)
...
## 6) Mobile-safe conflict reduction
- Use `MOBILE_CONFLICT_FREE_WORKFLOW.md` for a 4-file minimal edit strategy.
- Prefer splitting publish PRs into: (A) indexing-critical files, (B) promotional links/widgets.
